...
export function PublicHomePage() {
  ...
  useEffect(()=>{Promise.all([getPublicCommercialCatalog(organizationSlug),getOrganizationSettings(organizationSlug)]).then(([nextCatalog,nextOrganization])=>{setCatalog(nextCatalog);setOrganization(nextOrganization);}).catch(err=>setError(err instanceof Error?err.message:"No se pudo cargar la informacion.")).finally(()=>setLoading(false));},[organizationSlug]);
  ...
  return <div className="public-home" style={{"--brand-primary":organization?.primary_color||"#176f78","--brand-secondary":organization?.secondary_color||"#dff4ee"} as React.CSSProperties}>
    ...
    <main>
      ...
      <section className="public-home-band"><div className="public-home-section-head"><span>Especialidades</span><h2>Areas de atencion</h2></div><div className="public-home-grid">{catalog.specialties.map(item=><article key={item.id}><strong>{item.name}</strong><p>{item.description||"Atencion profesional con agenda disponible."}</p><a href={`/turnos?especialidad=${item.id}${organizationSlug?`&org=${encodeURIComponent(organizationSlug)}`:""}`}>Ver turnos</a></article>)}{!loading&&!catalog.specialties.length&&<p>Las especialidades se estan configurando.</p>}</div></section>
