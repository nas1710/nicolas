...
export function CommercialCatalogManager() {
  ...
  return <section className="panel admin-section wide commercial-manager">
    ...
    <details><summary>Prácticas ({data.practices.length})</summary><div className="commercial-rows">{data.practices.map(item => <PracticeCatalogRow key={item.id} item={item} specialty={data.specialties.find(s => s.id === item.specialty_id)?.name || ""} onSaved={refresh} />)}</div></details>
...
function PracticeCatalogRow({ item, specialty, onSaved }: { item: CommercialAdminCatalog["practices"][number]; specialty: string; onSaved: () => Promise<void> }) {
  const [form, setForm] = useState(item);
  return <div className="practice-catalog-row"><strong>{item.name}</strong><span>{specialty} · {item.duration_min} min</span><label><input type="checkbox" checked={form.requires_preparation || false} onChange={event => setForm({ ...form, requires_preparation: event.target.checked })} />Requiere indicaciones</label>{form.requires_preparation && <input value={form.preparation_instructions || ""} onChange={event => setForm({ ...form, preparation_instructions: event.target.value })} placeholder="Indicaciones previas" />}<label><input type="checkbox" checked={form.published} onChange={event => setForm({ ...form, published: event.target.checked })} />Publicada</label><label><input type="checkbox" checked={form.active} onChange={event => setForm({ ...form, active: event.target.checked })} />Activa</label><button onClick={async () => { await updateCommercialPractice(item.id, { published: form.published, active: form.active, requires_preparation: form.requires_preparation, preparation_instructions: form.preparation_instructions }); await onSaved(); }}>Guardar</button></div>;
}
